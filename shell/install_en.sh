...
allowPort() {
	...
	if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
		...
		if ! iptables -L | grep -q "$1(chuyik)"; then
			updateFirewalldStatus=true
			iptables -I INPUT -p tcp --dport "$1" -m comment --comment "allow $1(chuyik)" -j ACCEPT
		fi
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> Please refer to this tutorial for adding methods, https://github.com/chuyik/xray-agent/blob/master/documents/dns_txt.md"
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/chuyik/xray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
customCDNIP() {
	...
	echoContent yellow "# Notes"
	echoContent yellow "\nTutorial address:"
	echoContent skyBlue "https://github.com/chuyik/xray-agent/blob/master/documents/optimize_V2Ray.md"
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/chuyik/xray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/chuyik/xray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/chuyik/xray-agent/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/chuyik/xray-agent/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/chuyik/xray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:chuyik"; then
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "Any door unlock details, please check this article [https://github.com/chuyik/xray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
menu() {
	cd "$HOME" || exit
	echoContent red "\n=============================================================="
	echoContent green "author:chuyik"
	echoContent green "Current version: v2.6.10"
	echoContent green "Github:https://github.com/chuyik/xray-agent"
	echoContent green "Description: 8-in-1 coexistence script\c"
	showInstallStatus
	echoContent red "\n=============================================================="
	echoContent red "Promotion Area"
	echoContent green "AFF donation: https://github.com/chuyik/xray-agent/blob/master/documents/donation_aff.md\n"
