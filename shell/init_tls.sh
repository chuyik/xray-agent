...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/chuyik/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/chuyik/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/chuyik/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/chuyik/nginx/${domain}.crt --keypath /tmp/chuyik/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/chuyik/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/chuyik/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/chuyik/nginx"
    ls /tmp/chuyik/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/chuyik]"
    echoColor red "=============================="
